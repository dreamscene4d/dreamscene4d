...
parser.add_argument('--inpainted_dir', type=str, help="Path to inpainted RGB directory.")
parser.add_argument('--mask_dir', type=str, help="Path to mask directory.")
parser.add_argument('--inpainted_mask_dir', type=str, help="Path to inpainted mask directory.")
...
if __name__ == "__main__":
    ...
    # We splat the inpainted frames
    for obj_idx in range(1, num_objs+1):
        vid_dir = os.path.join(args.inpainted_dir, str(obj_idx).zfill(3))
        frames = glob.glob(vid_dir + '/*')
        frames.sort()
        frame0 = frames[0]
        frame0_name = os.path.basename(frame0)

        command = f'python main.py --config configs/image.yaml input={frame0} input_mask={os.path.join(args.inpainted_mask_dir, str(obj_idx).zfill(3), frame0_name)} outdir={args.out_dir} visdir={args.vis_dir} save_path={args.save_name}_{obj_idx}'
        subprocess.Popen(command, shell=True).wait()
    
    for obj_idx in range(1, num_objs+1):
        train_iters = len(frames) * 30 if args.lite else len(frames) * 100
        command = f'python main_4d.py --config configs/4d.yaml input={os.path.join(args.inpainted_dir, str(obj_idx).zfill(3))} input_mask={os.path.join(args.inpainted_mask_dir, str(obj_idx).zfill(3))} outdir={args.out_dir} visdir={args.vis_dir} save_path={args.save_name}_{obj_idx} iters={train_iters}'
        subprocess.Popen(command, shell=True).wait()

    # For composing, we use the original frames, the inpainted regions are not reliable to determine 3D scene structure
