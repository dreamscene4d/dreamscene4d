...
class Zero123(nn.Module):
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0, timesteps=None, clip_loss=False):
        ...
        with torch.no_grad():
            ...
            if timesteps is not None:
                ...
                cc_emb = torch.cat([self.embeddings[0].reshape(n_views, -1, C)[:, timesteps].reshape(-1, C).unsqueeze(1), T], dim=-1)
            else:
                cc_emb = torch.cat([self.embeddings[0].unsqueeze(1), T], dim=-1)
            cc_emb = self.pipe.clip_camera_projection(cc_emb)
            cc_emb = torch.cat([cc_emb, torch.zeros_like(cc_emb)], dim=0)
            ...
            if timesteps is not None:
                n_views = T.shape[0] // len(timesteps)
                C = self.embeddings[1].shape[-1]
                vae_emb = self.embeddings[1].reshape(n_views, -1, 4, C, C)[:, timesteps].reshape(-1, 4, C, C)
            else:
                vae_emb = self.embeddings[1]
            vae_emb = torch.cat([vae_emb, torch.zeros_like(vae_emb)], dim=0)
...
if __name__ == '__main__':
    ...
    while True:
        outputs = zero123.refine(image, elevation=[opt.elevation], azimuth=[opt.azimuth], radius=[opt.radius], strength=0)
        import ipdb
        ipdb.set_trace()
